...
public class AvailableCapturesForRook {
    ...
    private int numOfCaptures(int row, int column, char[][] board) {
        ...
        if (contains(board, row, column, true, true))
            numberOfCaptures++;
        if (contains(board, row, column, true, false))
            numberOfCaptures++;
        if (contains(board, row, column, false, true))
            numberOfCaptures++;
        if (contains(board, row, column, false, false))
            numberOfCaptures++;
    ...
    private boolean contains(char[][] board, int row, int column, boolean checkRow, boolean checkLeft) {
        int to = checkRow
                 ? (checkLeft ? column + 1 : board[0].length - column)
                 : (checkLeft ? row + 1 : board.length - row);
