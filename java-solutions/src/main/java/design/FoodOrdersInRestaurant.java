 ...
 */
public class FoodOrdersInRestaurant {

    public List<List<String>> displayTable(List<List<String>> orders) {
        Map<Integer, Map<String, Integer>> table = new HashMap<>();
        Set<String> dishes = new TreeSet<>(Comparator.naturalOrder());
        Set<Integer> tables = new TreeSet<>(Comparator.naturalOrder());
        for (List<String> order : orders) {
            int tableNumber = Integer.parseInt(order.get(1)); String dish = order.get(2);
            dishes.add(dish); tables.add(tableNumber);
            Map<String, Integer> tableSummary = table.computeIfAbsent(tableNumber, param -> new HashMap<>());
            tableSummary.put(dish, tableSummary.getOrDefault(dish, 0) + 1);
        }

        List<List<String>> result = new ArrayList<>();
        result.add(new ArrayList<>(dishes));
        ...
        for (int tableNumber : tables) {
            ...
            for (String dish : dishes) {
                String count = table.get(tableNumber).getOrDefault(dish, 0).toString();
                tableRow.add(count);
            }
            result.add(tableRow);
        }
