...
#ifndef C_ALGORITMS_DIVIDESTRINGINTOGROUPSOFSIZEK_H
#define C_ALGORITMS_DIVIDESTRINGINTOGROUPSOFSIZEK_H
...
public:
    vector<string> divideString(string s, int k, char fill) {
        int groupsCount = s.size() / k + (s.size() % k > 0 ? 1 : 0);
        vector<string> divided(groupsCount);

        for (int group = 0, index = 0; group < groupsCount; ++group, index += k)
            divided[group] = index + k < s.size() ? s.substr(index, k) : s.substr(index);

        while (divided[groupsCount - 1].size() < k)
...
#endif //C_ALGORITMS_DIVIDESTRINGINTOGROUPSOFSIZEK_H
