...
#ifndef C_ALGORITMS_INCREMENTALMEMORYLEAK_H
#define C_ALGORITMS_INCREMENTALMEMORYLEAK_H
...
At the ith second (starting from 1),
 i bits of memory are allocated to the stick with more available memory (or from the first memory stick if both have the same available memory).
 ...
 */
class IncrementalMemoryLeak {
public:
    vector<int> memLeak(int memory1, int memory2) {
        int bits;
        for (bits = 1; max(memory1, memory2) >= bits; bits++) {
            if (memory1 >= memory2)
                memory1 -= bits;
            else
                memory2 -= bits;
...
#endif //C_ALGORITMS_INCREMENTALMEMORYLEAK_H
