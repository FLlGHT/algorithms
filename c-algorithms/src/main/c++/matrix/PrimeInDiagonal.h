//
// Created by Michael Fedorenko on 09.04.2023.
//

#ifndef C_ALGORITHMS_PRIMEINDIAGONAL_H
#define C_ALGORITHMS_PRIMEINDIAGONAL_H
#include <vector>

using namespace std;

/**
 * 2614. Prime In Diagonal

You are given a 0-indexed two-dimensional integer array nums.

Return the largest prime number that lies on at least one of the diagonals of nums. In case, no prime is present on any of the diagonals, return 0.

Note that:

An integer is prime if it is greater than 1 and has no positive integer divisors other than 1 and itself.
An integer val is on one of thediagonals of nums if there exists an integer i for which nums[i][i] = val or an i for which nums[i][nums.length - i - 1]= val.


In the above diagram, one diagonal is [1,5,9] and another diagonal is [3,5,7].



Example 1:

Input: nums = [[1,2,3],[5,6,7],[9,10,11]]
Output: 11
Explanation: The numbers 1, 3, 6, 9, and 11 are the only numbers present on at least one of the diagonals. Since 11 is the largest prime, we return 11.
Example 2:

Input: nums = [[1,2,3],[5,17,7],[9,11,10]]
Output: 17
Explanation: The numbers 1, 3, 9, 10, and 17 are all present on at least one of the diagonals. 17 is the largest prime, so we return 17.
 */
class PrimeInDiagonal {

public:
    int diagonalPrime(vector<vector<int>>& nums) {
        int maxPrime = 0;
        for (int i = 0; i < nums.size(); ++i) {
            for (int j = 0; j < nums[0].size(); ++j) {
                bool inMainDiagonal = i == j;
                bool inAntiDiagonal = i == nums[0].size() - j - 1;

                if ((inMainDiagonal || inAntiDiagonal) && nums[i][j] > maxPrime && isPrime(nums[i][j]))
                    maxPrime = nums[i][j];
            }
        }

        return maxPrime;
    }

private:
    bool isPrime(int number) {
        if (number < 2)
            return false;

        for (int divider = 2; divider <= number / 2; ++divider) {
            if (number % divider == 0)
                return false;
        }

        return true;
    }
};


#endif //C_ALGORITHMS_PRIMEINDIAGONAL_H
